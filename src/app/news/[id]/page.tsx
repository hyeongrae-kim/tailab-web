...
export async function generateMetadata({ params }: NewsDetailPageProps): Promise<Metadata> {
  ...
  return {
    title: item.title,
    description: item.summary || "Lab news detail page.",
  };
}
    ...
    <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
      ...
      <section className="mt-8">
        {blocks.length > 0 ? (
          ...
          <p className="text-sm text-slate-600">Detailed block content will appear here once Notion data is connected.</p>
        )}
      </section>
    </article>
  );
